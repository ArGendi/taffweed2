...
import 'package:tafweed/services/dio/payment_services.dart';
...
class OrderCubit extends Cubit<OrderState> {
  ...
  void payNow(BuildContext context, Request request, String method) async{
    try{
      emit(LoadingPayState());
      log("payiiiiiiing");
      int? paymentId;
      double? rate = await CurrencyServices().get("AED");
      late double paidInUAE;
      if(rate != null){
        paidInUAE = double.parse(request.paid!)  * rate;
      }
      else{
        throw("currency error");
      }

      var apiKeyResponse = await PaymentServices().get();
      if(apiKeyResponse != null){
        FatoraahKeys.apiKey = apiKeyResponse.data["api_key"];
        Fatoraah.token = apiKeyResponse.data["api_token"];
      }
      else{
        emit(FailPayState());
      ...
      if(method == "Card"){
        paymentId = await Fatoraah.payWithCard(paidInUAE);
        log("caaaaaaaaaard");
      }
      else if(method == "Google"){
        paymentId = await Fatoraah.payWithGoogle(paidInUAE);
        log("Goooooogle");
      }
      else if(method == "Apple"){
        paymentId = await Fatoraah.payWithApple(paidInUAE);
        log("Apppppppple");
      }
