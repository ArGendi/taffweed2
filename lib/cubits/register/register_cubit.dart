...
class RegisterCubit extends Cubit<RegisterState> {
  ...
  void onRegister(BuildContext context) async{
    ...
    if(valid){
      ...
      var response = await authServices.register(name, requestPhone, password, Cache.getGender(), Cache.getCountryId()!, context);
      if(response.status == DataStatus.success){
        //print(response.data);
        emit(SuccessState());
        //regFormKey.currentState?.dispose();
        ScaffoldMessenger.of(context).showSnackBar(customSnackBar(AppLocalizations.of(context)!.registration_done_successfully_now_login, Colors.green));
        Navigator.pop(context);
      }
      else{
        emit(FailState());
        if(context.mounted){
          ScaffoldMessenger.of(context).showSnackBar(customSnackBar(response.content, Colors.red));
        }
      }
    }
  }
